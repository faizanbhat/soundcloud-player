...
import SoundCloudPlayer from '../src/SoundCloudPlayer'

configure({ adapter: new Adapter() })

SoundCloudPlayer.prototype.componentWillMount = function () {
  this.handlePlayerMount({ load: () => null })
}

test('refs', t => {
  const instance = shallow(<SoundCloudPlayer />).instance()
...
test('componentWillUnmount()', t => {
  const wrapper = shallow(<SoundCloudPlayer />)
...
test('getInternalPlayer()', t => {
  const wrapper = shallow(<SoundCloudPlayer />)
  wrapper.instance().player = 123
  t.true(wrapper.instance().getInternalPlayer() === 123)
})
...
test('player.load()', t => {
  const wrapper = shallow(<SoundCloudPlayer url='file.mp4' />)
...
test('set loadOnReady', t => {
  ...
  const wrapper = shallow(<SoundCloudPlayer url='file.mp4' activePlayer={() => null} />)
  const instance = wrapper.instance()
...
test('player.pause()', t => {
  const wrapper = shallow(<SoundCloudPlayer playing />)
  const load = sinon.fake()
...
test('player.setVolume()', t => {
  const wrapper = shallow(<SoundCloudPlayer volume={0.5} />)
  const load = sinon.fake()
...
test('player.mute()', t => {
  const wrapper = shallow(<SoundCloudPlayer muted={false} />)
  const load = sinon.fake()
...
test('player.unmute()', t => {
  const wrapper = shallow(<SoundCloudPlayer muted volume={0.8} />)
  const load = sinon.fake()
...
test('progress()', t => {
  ...
  const instance = shallow(<SoundCloudPlayer url='file.mp4' onProgress={onProgress} />).instance()
...
test('onReady()', t => {
  ...
  const setVolume = sinon.fake()
  const play = sinon.fake()
  const instance = shallow(<SoundCloudPlayer onReady={onReady} playing volume={1} />).instance()
...
test('onPlay()', t => {
  ...
  const instance = shallow(<SoundCloudPlayer onPlay={onPlay} />).instance()
  instance.handleDurationCheck = sinon.fake()
  instance.handlePlay()
...
test('onStart()', t => {
  ...
  const instance = shallow(<SoundCloudPlayer onStart={onStart} />).instance()
  instance.handleDurationCheck = sinon.fake()
...
test('seekOnPlay', t => {
  const seekTo = sinon.stub(SoundCloudPlayer.prototype, 'seekTo')
  const instance = shallow(<SoundCloudPlayer />).instance()
  instance.handleDurationCheck = sinon.fake()
...
test('onPause()', t => {
  ...
  const instance = shallow(<SoundCloudPlayer onPause={onPause} />).instance()
  instance.isLoading = false
...
test('onEnded()', t => {
  ...
  const instance = shallow(<SoundCloudPlayer activePlayer={activePlayer} onEnded={onEnded} />).instance()
...
test('handleDurationCheck', t => {
  ...
  const instance = shallow(<SoundCloudPlayer onDuration={onDuration} />).instance()
